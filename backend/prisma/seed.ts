...
function generateOrderDates(totalOrders: number): Date[] {
  ...
  for (let daysBack = 730; daysBack >= 0; daysBack--) {
...
async function main() {
  ...
  // Tạo từng batch 50 orders để tránh timeout
  const BATCH = 50;
  let created = 0;
  ...
  for (let i = 0; i < TOTAL_ORDERS; i += BATCH) {
    ...
    await Promise.all(
      batchDates.map(async (orderDate) => {
        const customer    = pick(customers);
        const rawStatus   = pick(statusPool);
        const status: 'PENDING' | 'DONE' = rawStatus === 'PENDING' ? 'PENDING' : 'DONE';
        const paymentStatus: PaymentStatus =
          status === 'PENDING' ? pick(['PENDING', 'PAID']) : 'PAID';
        const itemCount   = ri(1, 4);
    ...
    process.stdout.write(`\r   Orders created: ${created}/${TOTAL_ORDERS}`);
  }
  console.log(`\n✅ Created ${TOTAL_ORDERS} orders spread across 730 days.`);
