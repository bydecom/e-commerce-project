...
import { createOrderSchema, updateOrderStatusSchema, adminOrderQuerySchema, userOrderQuerySchema } from './order.schema';
import * as orderController from './order.controller';

export const orderRouter = Router();

orderRouter.post('/',               authMiddleware, validateBody(createOrderSchema), orderController.createOrder);
orderRouter.get('/me',              authMiddleware, validateQuery(userOrderQuerySchema), orderController.listMyOrders);
...
orderRouter.get('/',                authMiddleware, requireRole(['ADMIN']), validateQuery(adminOrderQuerySchema), orderController.listAdminOrders);
